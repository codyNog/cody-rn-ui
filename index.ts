...
const GitHubFileExtractor = (token: string) => {
    ...
    const findNearestPackageJson = async (startDir: string): Promise<string | null> => {
    ...
    const mergeDependencies = async (localPackageJsonPath: string): Promise<void> => {
    ...
    const extractSpecificDirectory = async (
        ...
        try {
            ...
            const targetPathInZip = `${rootDir}/${CONFIG.path}/`;
             const extractPath = path.join(outputPath, rootDir);
            ...
            const filteredEntries = entries.filter(entry => entry.entryName.startsWith(targetPathInZip));
              if (filteredEntries.length === 0) {
                 throw new Error(`No files found in ${CONFIG.path} directory within the zip file`);
              }

              const tempZip = new AdmZip();
             for (const entry of filteredEntries) {
                tempZip.addFile(entry.entryName.substring(targetPathInZip.length), entry.getData());
           }
